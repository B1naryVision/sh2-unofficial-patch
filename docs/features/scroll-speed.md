...
Scrolling the camera across the map — with the keyboard scroll keys or by
...
Multiply the camera's scroll speed constant where the controller turns the
frame time into a target velocity. This scales the whole motion profile
uniformly — the top speed and the acceleration ramp alike — for every scroll
trigger, keyboard and mouse edge both, while the map-bounds clamps still apply.

The factor comes from `[camera] ScrollSpeedMultiplier` in
...
## How scrolling works in the engine
...
- **Mouse edge scroll** (`0x5feeec`–`0x5fef17`): cursor x `<= 0` or
...
whole exe**, this one — so it is unambiguously the camera scroll speed, in world
...
- The displacement is finally scaled by `distance / 16000` (the camera's zoom
  distance at `+0x20`, divisor at VA `0x9f3210`), so **scrolling is faster the
...
| After | `d8 0d <abs32 → g_scrollSpeed>` (`fmul dword [g_scrollSpeed]`) |

`g_scrollSpeed` is a static `float` in the patch DLL holding
...
- **`Camera::pan` (VA `0x761f30`) is not the scroll path.** It looks like the
  obvious mover — it is the only function that writes the camera's `+0xc`/`+0x14`
  ...
  `1024 … mapTiles × 1024 − 1024`, and nudges it back. The real scroll runs
...
- `0x5fe010` also computes a rotation delta from its own frame-time product
  (`× 76800` at `0x5f9d86` is scroll; `× 131072 × 0.125` at `0x5f9ee5` is the
...
**Safe for version mismatch.** The camera position and its scroll velocity are
